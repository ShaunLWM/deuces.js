    ...
    multiples() {
        ...
        for (const tp of tpgen) {
            ...
            for (const kicker of kickers) {
                const product = Card.PRIMES[pair1] ** 2 * Card.PRIMES[pair2] ** 2 * Card.PRIMES[kicker];
                this._unsuited_lookup[product] = rank;
                rank += 1;
            }
        }
        ...
        for (const pairrank of backwards_ranks) {
            ...
            for (const kicks of kgen) {
                ...
                const product = Card.PRIMES[pairrank] ** 2 * Card.PRIMES[k1] * Card.PRIMES[k2] * Card.PRIMES[k3];
                this._unsuited_lookup[product] = rank;
                rank += 1;
            }
        }
